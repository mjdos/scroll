...
<label for="valor_eth">Valor do Mint:</label>
<input type="text" id="valor_eth" name="valor_eth">
<label for="walletAddress">Carteira que vai Comprar:</label>
...
<script>
    ...
    window.addEventListener('load', async () => {
        if (window.ethereum) {
            ...
            const contractAddress = "0xA139F36c970Db2Faf91f2E3148332611f4C52897";
    ...
    queryButton.addEventListener('click', () => {
        ...
        const valor_eth = document.getElementById('valor_eth').value;
        const tokenURI = document.getElementById('tokenURI').value;
        const walletAddress = document.getElementById('walletAddress').value;
     
        // Converter Ether para Wei
        const valorWei = web3.utils.toWei(valor_eth, 'ether');

        // Chamada ao método safeMint do contrato
        contract.methods.publicMint(tokenURI, valorWei).send({ from: walletAddress })
            .then(mintResponse => {
